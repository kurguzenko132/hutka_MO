...
export default async function HypothesesPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Гипотезы" subtitle="Проверка идей через данные, опросы, кампании и инсайты" actionLabel={can(role, 'manageHypotheses') ? 'Добавить гипотезу' : undefined} actionHref={can(role, 'manageHypotheses') ? '/hypotheses/new' : undefined} />
              ...
              <CardContent>
                <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-start">
                  <div className="min-w-0">
                    ...
                    <p className="mt-2 line-clamp-2 text-sm leading-6 text-app-muted">{item.description || item.testMethod || 'Описание гипотезы пока не добавлено.'}</p>
      ...
      {hypotheses.length === 0 && (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <h3 className="mt-4 text-xl font-black text-app-text">Гипотез пока нет</h3>
            <p className="mt-2 max-w-xl text-sm leading-6 text-app-muted">Создай первую гипотезу, чтобы проверить идею через кампании, опросы и реальные контакты.</p>
            {can(role, 'manageHypotheses') && <Button asChild className="mt-5"><Link href="/hypotheses/new"><Plus className="h-4 w-4" />Добавить гипотезу</Link></Button>}
          </CardContent>
        </Card>
      )}
        ...
        <CardContent className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm leading-6 text-app-muted">Гипотеза — это предположение. Кампания или опрос — проверка. Инсайт — вывод. Решение — изменение продукта, оффера или канала.</p>
            </div>
          </div>
