...
const demoRecommendation: LaunchRecommendation = {
  ...
  reason: 'В городе больше всего контактов, сильнее плотность мастеров и выше готовность к пилоту. Это лучший вариант, чтобы быстро собрать первую карту и проверить реальные заявки.',
  score: 88,
  nextActions: [
...
function buildRecommendation(cities: CitySummary[], nichesByCity: CityNicheSummary[]): LaunchRecommendation {
  ...
  return {
    ...
    reason: `${city} показывает лучшую плотность контактов и готовность к пилоту. Ниша “${niche}” выглядит наиболее перспективной для проверки карты и первых заявок.`,
    score,
    nextActions: [
      `Добрать контакты в связке “${city} + ${niche}” до минимальной плотности`,
      'Перевести горячих контактов в пилот и помочь им заполнить профиль',
