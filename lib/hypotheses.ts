...
const demoHypotheses: HypothesisDetail[] = [
  {
    ...
    successMetric: 'Оффер про клиентов должен дать минимум в 2 раза больше ответов и согласий на пилот.',
    ...
    result: 'Гипотеза подтверждается: в коммуникации нужно продавать не CRM, а поток новых заявок с карты.',
  ...
  },
  {
    ...
    successMetric: 'Минимум 4 из 10 салонов должны согласиться на мягкий пилот карты без переноса процессов.',
  ...
  },
  {
    ...
    description: 'Если онбординг занимает больше 5 минут, мастера будут откладывать заполнение профиля и не дойдут до пилота.',
    category: 'Онбординг',
    status: 'needs_data',
...
function mapHypothesis(row: Record<string, unknown>): HypothesisDetail {
  ...
  return {
    ...
    category: String(row.category ?? 'Гипотеза'),
    status,
